...
public class _33_DecimalConversion {

  /**
   * Return the fraction output in the following way Examples: If after decimal, repeating numbers
   * are there in the output . eg. 1/3=0.333333333, this should be represented as 0.(3)
   * 6/11=0.54545454, this should be represented as 0.(54) fractionRepresentation(1,2)=0.5
   * fractionRepresentation(1,3)=0.(3) fractionRepresentation(6,11)=0.(54)
   */
  public static String fractionRepresentation(int num, int den) {
    ...
    while (i < subString.length()) {
      if (intermediateSubString.length() > 0 && (i + intermediateSubString.length()
          < subString.length())
          && subString.substring(i, i + intermediateSubString.length())
          .equals(intermediateSubString)) {
        repeated = true;
        break;
